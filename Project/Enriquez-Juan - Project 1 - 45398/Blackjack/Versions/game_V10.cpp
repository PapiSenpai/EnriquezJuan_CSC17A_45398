...
void CheckSum(Player *,int,int,int,int,int);
...
int main(int argc, char** argv) {
    ...
    // Actions
    //
    rules.open("Rules.txt", ios::in); // opens a file and put it into input mode
    ...
    while(numOfP<1 || numOfP>4){ // add more later so that it only accepts ints
        if(numOfP==0)
        {
    ...
    for(i=0;i<numOfP;i++) //for loop used to enter name to each Player 
    {                     //structure array
        cin >> p[i].name;
    }
    //
 ...
 ******************************************************************************/
void CheckSum(Player *p,int numOfP, int a, int b, int c, int d)
{
    int *sum; // holds the sum of each players hand
    int i;
    int total=0;
    sum = new int[4];
    // Make an if statement that would limit which for loops to use
    if(numOfP>=1)
    {
        for(i=0;i<a;i++)
        {
