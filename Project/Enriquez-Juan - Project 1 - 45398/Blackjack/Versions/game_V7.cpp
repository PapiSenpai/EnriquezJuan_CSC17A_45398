...
Card draw(Deck &, int &);
void Dealer(Deck &, Player *, int &, int);
int CheckWin(Deck, Card, Player *,int,int);
...
void Dealer(Deck &d, Player *p, int &index,int &a)
{
    int choice;
    int i=2,j=2,k=2,l=2; // Counters
    ...
    while(choice!=5){
        ...
        if(choice==1)
        {
            p[0].hand[i] = draw(d, index);
            i++;
        }
        else if(choice==2)
        {
            p[1].hand[j] = draw(d, index);
            j++;
        }
        else if(choice==3)
        {
            p[2].hand[k] = draw(d, index);
            k++;
        }
        else if(choice==4)
        {
            p[3].hand[l] = draw(d, index);
            l++;
        }
        else
        {
...
int CheckWin(Deck d, Card c, Player *p,int numOfP, int &a)
{
