...
void PrintAll(Deck d)
{
    for(int col=(int)Suits::Clubs; col<=(int)Suits::Spades; col++)
    {
        for(int row=(int)CardName::Ace; row <= (int)CardName::King; row++)
        {
            ...
            d.PrintCard(d.arrCards[index]); // Prints contents in a card struct 
        }
    }
